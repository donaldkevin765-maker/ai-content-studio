...
class TextToSpeechService:
    async def generate(self, text: str, output_path: str, lang: Optional[str] = None) -> str:
        ...
        if engine == "coqui":
            ...
            # Default: Edge-TTS (gratuito, voci neurali, 400+ voci, 50+ lingue)
            return await self._edge_tts(text, str(out), lang)

    async def _edge_tts(self, text: str, output_path: str, lang: str) -> str:
        try:
            import edge_tts
            # Mappa lingua a voce Edge-TTS
            voice_map = {
                "it": "it-IT-ElsaNeural",
                "en": "en-US-AriaNeural",
            ...
            voice = voice_map.get(lang[:2], "it-IT-ElsaNeural")
            communicate = edge_tts.Communicate(text, voice)
            await communicate.save(output_path)
            logger.info(f"Audio Edge-TTS: {output_path} (voce={voice})")
            return output_path
        except ImportError:
            logger.warning("edge-tts non installato, fallback gTTS")
            return await self._gtts(text, output_path, lang)
        except Exception as e:
            logger.error(f"Edge-TTS errore: {e}, fallback gTTS")
            return await self._gtts(text, output_path, lang)

    async def _elevenlabs(self, text: str, output_path: str, lang: Optional[str] = None) -> str:
        try:
