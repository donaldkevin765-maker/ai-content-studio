...
class ScriptGenerator:
    ...
    async def generate(self, topic: str, duration_sec: int = 60, style: str = "informativo", scene_count: int = 0) -> dict:
        logger.info(f"Generazione script: topic={topic}, durata={duration_sec}s, stile={style}, scene={scene_count}")
        ...
        for provider in provider_order:
            try:
                if provider == "ensemble":
                    ...
                    result = await LLMEnsemble().generate(self._build_prompt(topic, duration_sec, style, scene_count))
                    if result:
                        return self._parse_llm_result(result, topic, duration_sec, style, scene_count)
                elif provider == "ollama" and settings.ollama_base_url:
                    return await self._generate_with_llm(topic, duration_sec, style, scene_count)
            except Exception as e:
                logger.warning(f"Provider {provider} fallito: {e}")
                continue

        return self._generate_template(topic, duration_sec, style, scene_count)

    def _get_scene_count(self, duration_sec: int, scene_count: int) -> int:
        if scene_count > 0:
            return scene_count
        return max(2, min(8, duration_sec // 10))

    def _build_prompt(self, topic: str, duration_sec: int, style: str, scene_count: int = 0) -> str:
        num_scenes = self._get_scene_count(duration_sec, scene_count)
...
La durata totale di tutte le scene deve essere circa {duration_sec} secondi."""
    ...
    def _parse_llm_result(self, result: str, topic: str, duration_sec: int, style: str, scene_count: int = 0) -> dict:
        import re as _re
        num_scenes = self._get_scene_count(duration_sec, scene_count)
        try:
    ...
    async def _generate_with_llm(self, topic: str, duration_sec: int, style: str, scene_count: int = 0) -> dict:
        prompt = self._build_prompt(topic, duration_sec, style, scene_count)
    ...
    def _generate_template(self, topic: str, duration_sec: int, style: str, scene_count: int = 0) -> dict:
        import random
