...
def video_output_path(filename: Optional[str] = None) -> str:
    return str(VIDEOS_DIR / (filename or unique_filename(".mp4")))


def audio_output_path(filename: Optional[str] = None) -> str:
    return str(AUDIO_DIR / (filename or unique_filename(".mp3")))


def image_output_path(filename: Optional[str] = None) -> str:
